...
const OrdersPage = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await fetch(
        "http://ec2-13-201-203-2.ap-south-1.compute.amazonaws.com:8080/shawarmahouse/v1/status"
      );
      if (!response.ok) {
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    ...
    try {
      const response = await fetch(
        "http://ec2-13-201-203-2.ap-south-1.compute.amazonaws.com:8080/shawarmahouse/v1/updateOrderStatus",
        requestOptions
      );
      if (!response.ok) {
  ...
  const removeOrder = async (orderId) => {
    try {
      const response = await fetch(
        `http://ec2-13-201-203-2.ap-south-1.compute.amazonaws.com:8080/shawarmahouse/v1/removeOrder/${orderId}`,
        {
          method: "DELETE",
        }
      );
